...
//    this->move(width - this->width(), height - this->height());
    ...
    this->setWindowTitle("整點報時！");

    ui.label->setStyleSheet(QString("border-image:url(:/AlarmClock/Resources/bk.png);background-image:url(:/AlarmClock/Resources/bk.png);color: rgb(170, 0, 0);font: 16pt \"華文中宋\";"));

    QString strTime = QTime::currentTime().toString("HH:mm");
    if (QTime::currentTime().hour() < 12)
    {
        ui.label->setText("現在是：上午" + strTime);
    }
    else if (QTime::currentTime().hour() == 12)
    {
        ui.label->setText("現在是：中午" + strTime);
    }
    else if (QTime::currentTime().hour() > 12)
    {
        ui.label->setText("現在是：下午" + strTime);
    }
